...
class C_order extends CI_Controller {
	...
	public function index()
	{
		if ($this->session->userdata('akses') == 1) {
            ...
            $this->load->view('page/admin/order/v_order',$data);
    ...
    public function deleteOrder($id)
    {
        if ($this->session->userdata('akses') == 1) {
            ...
            if ($delete) {
                $this->session->set_flashdata('msg','Data Berhasil DiHapus!');
                redirect('admin/c_order/order/'.$_GET['order']);
            }else{  
                $this->session->set_flashdata('msg','Data Gagal DiHapus!');
                redirect('admin/c_order/order/'.$_GET['order']);
            }
    ...
    public function edit($id)
    {
        if ($this->session->userdata('akses') == 1) {
            ...
            if ($this->input->post('update')) {
                if ($this->input->post('id_meja') == $_GET['meja']) {
                    $data = [
                        "id_user" => $this->session->userdata('id_user'),
                        "no_meja" => $_GET['meja'],
                        "keterangan" =>  htmlspecialchars($this->input->post('keterangan',true)),
    ...
    public function order($id)
    {
        ...
        $data['title'] = "Order Detail - Restoran";
            if ($this->input->post('simpan')) {
                ...
                if ($save) {
                    $this->session->set_flashdata('msg','Data Berhasil DiUpdate!');
                    redirect('admin/c_order/order/'.$id);
                }else{  
                    $this->session->set_flashdata('msg','Data Gagal DiUpdate!');
                    redirect('admin/c_order/order/'.$id);
                }
        ...
        $this->load->view('partial/header',$data);
        $this->load->view('page/admin/order/v_orderDetail',$data);
        $this->load->view('partial/footer');
    }
