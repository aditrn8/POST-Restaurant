            ...
            <div class="card-body">
                <div class="table-responsive">
                    <table class="table table-hover table-stripped" id="dataTable">
                        ...
                        <tbody>
                            <?php $no=1;foreach ($list_order as $a) :?>
                            <tr>
                                <td><?= $no++ ?></td>
                                <td><?= $a['no_meja']?></td>
                                <td><?= $a['tanggal']?></td>
                                <td><?= $a['keterangan']?></td>
                                <td>
                                    <a href="<?= base_url('waiter/c_order/delete/'.$a['id_order'].'/?meja='.$a['no_meja']) ?>" onclick="return confirm('Yakin ingin menghapus data? Data yang dihapus tidak bisa dikembalikan.')" class="btn btn-danger"><i class="fa fa-trash"></i></a>
                                    <a href="<?= base_url('waiter/c_order/order/'.$a['id_order']) ?>" class="btn btn-warning"><i class="fa fa-shopping-cart"></i></a>
                                </td>
                            </tr>
                            <?php endforeach;?>
                        </tbody>
